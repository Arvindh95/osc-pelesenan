import { ReactNode, useState, useEffect } from 'react';
import { createAlertAria, generateId } from '../../utils/accessibility';
import { useScreenReader } from '../../hooks/useAccessibility';

export interface AlertProps {
  type: 'success' | 'error' | 'warning' | 'info';
  message: string | ReactNode;
  dismissible?: boolean;
  onDismiss?: () => void;
  className?: string;
  size?: 'sm' | 'md' | 'lg';
  autoAnnounce?: boolean;
  live?: boolean;
}

const Alert = ({
  type,
  message,
  dismissible = false,
  onDismiss,
  className = '',
  size = 'md',
  autoAnnounce = true,
  live = true,
}: AlertProps) => {
  const [isVisible, setIsVisible] = useState(true);
  const { announce } = useScreenReader();
  const alertId = generateId('alert');

  const handleDismiss = () => {
    setIsVisible(false);
    onDismiss?.();
    if (autoAnnounce) {
      announce('Alert dismissed', 'polite');
    }
  };

  // Announce alert to screen readers when it appears
  useEffect(() => {
    if (autoAnnounce && typeof message === 'string') {
      const priority = type === 'error' ? 'assertive' : 'polite';
      announce(`${type} alert: ${message}`, priority);
    }
  }, [autoAnnounce, message, type, announce]);

  if (!isVisible) return null;

  const getTypeClasses = (): string => {
    const types = {
      success: 'bg-green-50 border-green-200 text-green-800',
      error: 'bg-red-50 border-red-200 text-red-800',
      warning: 'bg-yellow-50 border-yellow-200 text-yellow-800',
      info: 'bg-blue-50 border-blue-200 text-blue-800',
    };

    return types[type];
  };

  const getIconClasses = (): string => {
    const iconColors = {
      success: 'text-green-400',
      error: 'text-red-400',
      warning: 'text-yellow-400',
      info: 'text-blue-400',
    };

    return iconColors[type];
  };

  const getSizeClasses = (): string => {
    const sizes = {
      sm: 'p-3 text-sm',
      md: 'p-4 text-sm',
      lg: 'p-6 text-base',
    };

    return sizes[size];
  };

  const getIcon = (): ReactNode => {
    const iconClass = `w-5 h-5 ${getIconClasses()}`;

    switch (type) {
      case 'success':
        return (
          <svg
            className={iconClass}
            fill="currentColor"
            viewBox="0 0 20 20"
            aria-hidden="true"
          >
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
              clipRule="evenodd"
            />
          </svg>
        );
      case 'error':
        return (
          <svg
            className={iconClass}
            fill="currentColor"
            viewBox="0 0 20 20"
            aria-hidden="true"
          >
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
              clipRule="evenodd"
            />
          </svg>
        );
      case 'warning':
        return (
          <svg
            className={iconClass}
            fill="currentColor"
            viewBox="0 0 20 20"
            aria-hidden="true"
          >
            <path
              fillRule="evenodd"
              d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
              clipRule="evenodd"
            />
          </svg>
        );
      case 'info':
        return (
          <svg
            className={iconClass}
            fill="currentColor"
            viewBox="0 0 20 20"
            aria-hidden="true"
          >
            <path
              fillRule="evenodd"
              d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
              clipRule="evenodd"
            />
          </svg>
        );
      default:
        return null;
    }
  };

  const alertClasses = `
    border rounded-md
    ${getTypeClasses()}
    ${getSizeClasses()}
    ${className}
  `
    .trim()
    .replace(/\s+/g, ' ');

  const ariaAttributes = createAlertAria(type, live);

  return (
    <div id={alertId} className={alertClasses} {...ariaAttributes}>
      <div className="flex items-start">
        <div className="flex-shrink-0">{getIcon()}</div>

        <div className="ml-3 flex-1">
          <div className="text-sm">
            {typeof message === 'string' ? <p>{message}</p> : message}
          </div>
        </div>

        {dismissible && (
          <div className="ml-auto pl-3">
            <div className="-mx-1.5 -my-1.5">
              <button
                type="button"
                onClick={handleDismiss}
                className={`
                  inline-flex rounded-md p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-2
                  ${type === 'success' ? 'text-green-500 hover:bg-green-100 focus:ring-green-600' : ''}
                  ${type === 'error' ? 'text-red-500 hover:bg-red-100 focus:ring-red-600' : ''}
                  ${type === 'warning' ? 'text-yellow-500 hover:bg-yellow-100 focus:ring-yellow-600' : ''}
                  ${type === 'info' ? 'text-blue-500 hover:bg-blue-100 focus:ring-blue-600' : ''}
                `}
                aria-label={`Dismiss ${type} alert`}
                aria-describedby={alertId}
              >
                <svg
                  className="w-5 h-5"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  aria-hidden="true"
                >
                  <path
                    fillRule="evenodd"
                    d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                    clipRule="evenodd"
                  />
                </svg>
              </button>
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

export default Alert;
